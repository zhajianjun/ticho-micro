...
public class DictServiceImpl implements DictService {
    ...
    private void fillDict(Dict dict, Dict parent) {
        ...
        if (Objects.isNull(parent)) {
            ...
            dict.setPid(CommConst.DICT_PARENT_ID);
            dict.setStatus(1);
        } else {
    ...
    private Dict getParentAndPreCheck(Dict dict) {
        ...
        Long pid = Optional.ofNullable(dict.getPid()).orElse(CommConst.DICT_PARENT_ID);
        ...
        query.eq(Dict::getIsDelete, false);
        // 同类型、同个级别的字典的code不能重复
        Assert.isTrue(dictRepository.count(query) == 0, BizErrCode.FAIL, "该数据字典重复");

        // pid=-1则表示是根节点
        if (Objects.equals(CommConst.DICT_PARENT_ID, pid)) {
            return null;
        }
        // 如果不是根节点，则需要查下pid是否存在
        Dict parent = dictRepository.getById(pid);
        // 父节点删除情况下无法添加
        Assert.isTrue(parent != null && parent.getIsDelete() == 0, BizErrCode.FAIL, "该字典的父节点不存在，或已经删除");
        return parent;
    }
